...
# File path to store test results, best models & features
os.makedirs('models/rf_results', exist_ok=True)

train_df = pd.read_csv('data/interim/twitter_train_processed.csv', index_col=0)
val_df = pd.read_csv('data/interim/twitter_val_processed.csv', index_col=0)
...
X_val = val_df.drop(columns=['account_type'])
y_val = val_df['account_type']

X_test = test_df.drop(columns=['account_type'])
y_test = test_df['account_type']
...
def evaluate(model, X, y, label="Validation"):
    ...
    plt.tight_layout()    
    plt.savefig(f"models/rf_results/{label.lower().replace(' ', '_')}_confusion_matrix.png")
    plt.close()
    ...
    plt.tight_layout()       
    plt.savefig(f"models/rf_results/{label.lower().replace(' ', '_')}_roc_curve.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"models/rf_results/{label.lower().replace(' ', '_')}_pr_curve.png")
    plt.close()    
...
for combo in itertools.product(
    ...
    safe_lbl = safe_label(f"n{params['n_estimators']}_d{params['max_depth']}_s{params['min_samples_split']}_l{params['min_samples_leaf']}")
...
joblib.dump(refitted_model, 'models/rf_results/best_rf_rfe.pkl')
joblib.dump(best_features, 'models/rf_results/best_features_rfe.pkl')
